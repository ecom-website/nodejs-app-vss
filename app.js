...
console.log(__dirname+"/images/vedhalogo.jpg");
*/
app.get('/vedhatechnologies', function(request, response) {
    //response.send("<h2><center>Welcome to Node JS app</h2>");
    response.write("<h2><center><u>Node JS  Application </u></center></h2>");
	
    response.write("<h2><center>Welcome to  Vedha Technologies. Please Contact +91-87922084 for more information or send an email to devopstrainingind@gmail.com <center></h2>" );
    response.end();
...
app.get("/html", function(req,res){
    ...
    app.get("/jsonData", function(req,res){
        ...
        //res.json({'name': 'Vedha T'});
        res.send({
		'name': 'Vedha Technologies',
		'technology': 'DevOps',
		'contact' : '87922084',
		'email': 'devopstrainingind@gmail.com'
	            });
...
app.get('/redirect', function(req, res) {
    //Send status 300
        res.redirect('http://vedhatechnologies.com');
    });
    
 
    app.listen(app.get('port'), function() {
        console.log("Node JS app is running at http://localhost:" + app.get('port') +"/vedhatechnologies");
      })
